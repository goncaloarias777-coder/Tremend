...
import { Store, Save, ArrowLeft, ExternalLink, Globe, CheckCircle2 } from 'lucide-react'
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {slug && (
            ...
            >
              <Globe className="w-4 h-4" /> Ver mi Tienda <ExternalLink className="w-3.5 h-3.5" />
            </a>
          )}
        </div>

        <div className="bg-slate-800 border border-slate-700/80 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
          <div className="flex items-center gap-3 pb-6 border-b border-slate-700/60">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Personaliza los datos públicos y vincula tus cobros</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <button 
              type="submit" 
              disabled={saving}
              className="w-full bg-amber-400 hover:bg-amber-300 text-slate-900 font-bold py-3 px-4 rounded-xl transition flex items-center justify-center gap-2 text-sm shadow-md shadow-amber-400/20 cursor-pointer disabled:opacity-50"
            >
              <Save className="w-4 h-4" /> {saving ? 'Guardando...' : 'Guardar y Continuar'}
            </button>
          </form>

          {/* Sección de Mercado Pago OAuth */}
          <div className="pt-6 border-t border-slate-700/60">
            <h2 className="text-sm font-bold text-white mb-1">Pasarela de Cobros (Mercado Pago)</h2>
            <p className="text-xs text-slate-400 mb-4">Vincula tu cuenta para recibir los pagos de tus ventas directamente.</p>
            ...
            {store?.mercadopago_access_token ? (
              <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 p-3.5 rounded-xl text-xs font-semibold flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" /> ¡Cuenta de Mercado Pago vinculada correctamente!
              </div>
            ) : (
              <a 
                href="/api/mp/connect"
                className="w-full bg-[#009EE3] hover:bg-[#0081BE] text-white font-bold py-3 px-4 rounded-xl transition flex items-center justify-center gap-2 text-sm shadow-lg shadow-sky-500/20 cursor-pointer"
              >
